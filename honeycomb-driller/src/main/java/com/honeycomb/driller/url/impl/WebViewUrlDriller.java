...
public class WebViewUrlDriller extends BaseUrlDriller {
    ...
    public static class Options {
        ...
        public int redirectTimeout;

        // Wait for a moment when page loading finished because of the refresh delay.
        public int finishedDelay;

        // Timeout milliseconds when loading page source.
        public int loadSourceTimeout;

        public Options() {
            this.redirectTimeout = 30000; // 30 seconds
            this.finishedDelay = 15000; // 15 seconds
            this.loadSourceTimeout = 10000; // 10 seconds
        }
    }
    ...
    public WebViewUrlDriller setOptions(Options options) {
        mOptions = options;
        return this;
    }
    ...
    public void onStartDrill(final String url) {
        ...
        scheduleTimeout(mOptions.redirectTimeout);
    }
    ...
    private class DrillWebViewClient extends WebViewClient {
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            scheduleOnFinish(mOptions.finishedDelay);
        }
            ...
            if (isLoadSource()) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        scheduleTimeout(mOptions.loadSourceTimeout);
                    }
                });
            } else {
                onFinished(null);
...
//            HLog.v(TAG, " - Done. source length: " + StringUtils.emptyIfNull(responseString).length());
            HLog.v(TAG, " - Done. source: " + responseString);
        } else {
            HLog.v(TAG, " - Done.");
        }
